...
function applyUI(){
  ...
  setText('noticeEyebrow',x.notice);setText('noticeTitle',x.brother);setText('hadithIntro',x.hadithIntro);setText('hadithGrade',x.hadithGrade);setText('hadithTranslationTitle',x.hadithTranslationTitle);setText('noticeText',x.noticeText);setText('goLive',x.understand);
...
function render(q=''){
  ...
  $('grid').querySelectorAll('.lang').forEach(b=>b.addEventListener('click',()=>{chosen=b.dataset.c;uiLang=(chosen==='ar'?'ar':'en');setText('langName',langs[chosen]);$('out').textContent='';$('out').dataset.empty='yes';$('interim').textContent='';applyUI();go('notice');loadHadith();}));
}
...
async function loadHadith(){
  const box=$('hadithTranslation');
  if(!box)return;
  if(chosen==='ar'){box.textContent='الحديث مذكور بالعربية في الأصل.';return;}
  box.textContent=t().hadithLoading;
  try{
    const r=await fetch('/api/hadith/',{method:'POST',headers:{'Content-Type':'application/json','X-Requested-With':'XMLHttpRequest'},body:JSON.stringify({target:chosen})});
    const data=await r.json();
    if(!r.ok||!data.text)throw new Error(data.error||t().hadithError);
    box.textContent=data.text;
  }catch(e){console.error(e);box.textContent=t().hadithError;}
}
